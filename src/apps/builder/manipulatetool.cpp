...
void ManipulateTool::mouseMoveEvent(QMouseEvent *event)
{
    if(state() == MovingSelection){
        ...
        if(event->buttons() & Qt::LeftButton){
            editor()->setAtomPosition(atom, view()->unproject(event->x(), event->y(), atom->position()).toPoint());
        }
        else if(event->buttons() & Qt::RightButton){
            int dy = event->y() - m_lastPosition.y();

            chemkit::GraphicsPoint position = atom->position();
            position.moveBy(dy * 0.1, -view()->camera()->direction());
            editor()->setAtomPosition(atom, position.toPoint());
        }
    }
