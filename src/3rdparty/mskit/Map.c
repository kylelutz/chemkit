...
static MapType *_MapNew(MSKContext * G, float range, float *vert, int nVert,
...
float MapGetSeparation(MSKContext * G, float range, float *mx, float *mn,
                       float *diagonal)
{
  float maxSize;
...
MapType *MapNew(MSKContext * G, float range, float *vert, int nVert, float *extent)
{
  return (_MapNew(G, range, vert, nVert, extent, NULL, -1, 0));
}

MapType *MapNewCached(MSKContext * G, float range, float *vert, int nVert,
                      float *extent, int group_id, int block_id)
{
  return (_MapNew(G, range, vert, nVert, extent, NULL, group_id, block_id));
}

MapType *MapNewFlagged(MSKContext * G, float range, float *vert, int nVert,
                        ...
                        float *extent, int *flag, int group_id, int block_base)
{
  ...
  PRINTFD(G, FB_Map)
    " MapSetup: %8.3f %8.3f %8.3f %8.3f %8.3f %8.3f\n",
  ...
  PRINTFD(G, FB_Map)
    " MapSetup: %8.3f %8.3f %8.3f %8.3f %8.3f %8.3f\n",
           I->Min[0], I->Min[1], I->Min[2], I->Max[0], I->Max[1], I->Max[2] ENDFD;
  PRINTFD(G, FB_Map)
    " MapSetup: %8d %8d %8d\n", I->Dim[0], I->Dim[1], I->Dim[2] ENDFD;
